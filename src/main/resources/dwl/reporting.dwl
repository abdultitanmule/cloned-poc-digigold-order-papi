...
flatten(payload.items map using ( item = $, rootIndex = $$) (
     addIndex(rootIndex, rootIndex + 1) map {
        ...
        FIELD14:"TQ",
        FIELD15:"00",
        FIELD16:getFinancialYear(item.LASTUPDATED as LocalDateTime {format: "M/d/yyyy H:m:s"} as Date), 
        FIELD17:item.LASTUPDATED as LocalDateTime {format: "M/d/yyyy H:m:s"} as String{format: "dd-MMM-yy"},
        FIELD18:"DIGIGOLD",
        FIELD19:item.LASTUPDATED as LocalDateTime {format: "M/d/yyyy H:m:s"} as String{format: "dd-MMM-yy"},
        FIELD20:1058,
        FIELD21:"A",
        FIELD22:"4265",
        FIELD23:(if(item.TRANSACTION_STATUS ~= "buy confirmed" and $$ == 0) 
				("UEC" ++ item.LASTUPDATED as LocalDateTime {format: "M/d/yyyy H:m:s"} as String {format: "ddMMyyyy"} ++ "_Online")
            else if (item.TRANSACTION_STATUS ~= "buy confirmed" and $$ == 1)
                (item.TRANSACTION_ID)
        ...
        FIELD26:rootIndex + $,
        FIELD27:"UEC",
        FIELD28:item.LASTUPDATED as LocalDateTime {format: "M/d/yyyy H:m:s"} as String{format: "dd-MMM-yy"},
